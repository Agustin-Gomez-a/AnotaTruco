...
export default function MatchBoard({ score, target, counterStyle = 'fosforos' }: MatchBoardProps) {
  ...
  const renderGroupList = (pts: number, maxPts: number) => {
    ...
    if (list.length === 0) {
      return (
        <div className="h-10 sm:h-12 flex items-center justify-center text-[10px] sm:text-xs tracking-wider font-semibold text-emerald-300/40 uppercase">
          <span>0 / {maxPts} pts</span>
        </div>
      );
    }

    return (
      <div className="flex flex-wrap justify-center content-start gap-0.5 sm:gap-1.5 min-h-[40px] sm:min-h-[52px] p-0.5 sm:p-1 transition-all">
        {list.map((cnt, idx) => (
  ...
  if (!hasHalves) {
    return (
      <div className="flex-1 rounded-xl p-1.5 sm:p-2 overflow-y-auto flex flex-col justify-between bg-black/30 border border-emerald-950/60">
        <div className="flex items-center justify-between px-1.5 mb-0.5">
          <span className="text-[10px] sm:text-[11px] font-bold tracking-widest text-amber-200/70 uppercase">
            Puntos
          </span>
        ...
        <div className="w-full bg-black/40 h-1 rounded-full overflow-hidden mt-1 border border-white/5">
          <div
  ...
  return (
    <div className="flex-1 flex flex-col gap-1.5 sm:gap-2 overflow-y-auto">
      {/* Sección MALAS */}
      <div
        className={`board-section rounded-xl p-1 sm:p-1.5 transition-all ${
      ...
      >
        <div className="flex items-center justify-between px-1 mb-0.5">
          ...
          <span className="text-[10px] sm:text-[11px] font-mono font-bold text-amber-200/90">
            {scoreInMalas}/{halfTarget}
          </span>
        </div>
      ...
      >
        <div className="flex items-center justify-between px-1 mb-0.5">
          ...
          <span className="text-[10px] sm:text-[11px] font-mono font-bold text-sky-200/90">
